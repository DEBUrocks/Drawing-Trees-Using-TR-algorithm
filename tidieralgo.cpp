...
*/
int Random_num(int l, int u)//picked up from stackoverflow
{
	return rand() % (u - l + 1) + l;
}
...
//Set up for subtree pushing. Place roots of subtrees minimum distance apart
			...
			{
				if (CURsep < minimum_sep) 
				{
					ROOTsep = ROOTsep + (minimum_sep - CURsep);
					CURsep = minimum_sep;
				}
				//Advance left 
				if (left->rlink != NULL) 
...
void tree::drawTree(int cx = 0, int cy = 0, node *N = NULL)
{
	...
	p.use_circle(dx, dy, radii);
	p.draw_circle();
	p.use_line(dx, dy, cx, cy); 
	p.draw_line();
}
...
void tree::Tidy(void)
{
	...
	while (current != NULL)
	{
		...
		case 2: //right visit
			...
			{
				place = current->rlink->x - minimum_sep;
			}
			else if (current->rlink == NULL)
			{
				place = current->llink->x + minimum_sep;
			}
			else
			{
			...
			next_pos[h] = current->x + 2*minimum_sep;
